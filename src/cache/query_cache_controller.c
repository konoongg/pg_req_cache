...
static proc_status process_write_db(connection* conn);
static proc_status notify_db(connection* conn);

db_worker dbw;
extern config_cache config;


static void free_db_command(command_to_db* cmd) {
...
static command_to_db* get_command(void) {
    command_to_db* cmd;
    dbw_lock();
    if (dbw.commands->first == NULL) {
        dbw_unlock();
...
static void* start_db_worker(void*) {
    while (true) {
